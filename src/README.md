...
* Генерация машинного кода в случае, если предыдущие этапы завершились без
  ошибок [Translator.py](translator/translator.py)
...
3. Для возможности подсчёта больших выражений унарные операции перезаписывают ячейку буфера.
   Используемые при подсчёте значения переменных предварительно выгружаются в буффер.
...
`python3 sovcode.py <путь_до_бинарного_файла> <аргументы>[...]`

### Входные данные

Имя бинарного файла, аргументы программы

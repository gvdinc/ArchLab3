...
| 7   |   cmp	    |    DmemArg	    |    2    | сравнить код из аккумулятора c ячейкой памяти, выставить регистры | 	z = (dmem[arg] == acc); c = (acc < dmem[arg])   |
...
| 11  |   btn	    |    NoneArg	    |   1	    |                         побитовое не acc	                         | ~acc                                             |
| 12  |   xor	    |    DmemArg	    |   1	    |                        логический xor acc	                        | acc xor dmem[arg]	                               |
| 13  |   jmp	    |    PmemArg	    |   1	    |                    совершить переход на pmem	                     | pc = pmem[arg]                                   |
| 14  |    jz	    |    PmemArg	    |   1	    |          переход на pmem при установленом zero флаге 1	           | z == 1 ? pc = pmem[arg]                          |
| 15  |   jnz	    |    PmemArg	    |    	    |         переход на pmem при неустановленом zero флаге 0	          | z == 0 ? pc = pmem[arg]	                         |
| 16  |   mult	   |    DmemArg	    |   2	    |          умножить аккумулятор на знанч. из ячейки пам.	           | acc = acc * dmem[arg]                            |
| 17  |   dev	    |    DmemArg	    |   2	    |          разделить аккумулятор на знанч. из ячейки пам.	          | acc = acc / dmem[arg]                            |
| 18  |   mod	    |    DmemArg	    |   2	    |         остаток от деления аккумулятора на знанч.ячейки	          | acc = acc % dmem[arg]                            |
| 19  |   pow	    |    DmemArg	    |   2 	   |          возвести аккумулятор в степень из ячейки пам.	           | acc ^ dmem[arg]                                  |
| 1A  |   sqrt	   |    NoneArg	    |   2	    |               подсчитать корень знач. аккумулятора	               | sqrt(acc)	                                       |
| 1B  |    in	    |    IOport	     |   2	    |           Ввод символьного значения с внешнего устр-ва	           | input >> dmem[arg]                               |
| 1C  |   out	    |    IOport	     |   2 	   |          Вывод символьного значения на внешнее устр-во	           | dmem[arg] >> output                              |
...
6. Макс. длина строки 255 символов -> 255 регистров (+1 служебный регистр). Как в pascal. Именно столько выделит аллокатор
